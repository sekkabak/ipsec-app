...
class DiffieHellman(object):
    ...
    def generate_private_key(self, n):
        self.__private_key = int(binascii.hexlify(os.urandom(n)), 16)

    def generate_public_key(self):
        self.__public_key = pow(self.generator, self.__private_key,
                              PRIMES[self.group])

    def derivate(self, other_key) -> bytes:
        self.__shared_secret = pow(other_key, self.__private_key, PRIMES[self.group])
        return self.shared_secret

    @property
    def public_key(self) -> bytes:
        return to_bytes(self.__public_key)
    
    @property
    def private_key(self) -> bytes:
        return to_bytes(self.__private_key)

    @property
    def shared_secret(self) -> bytes:
        return to_bytes(self.__shared_secret)
