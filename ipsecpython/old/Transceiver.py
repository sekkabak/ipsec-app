...
class Transceiver:
    ...
    async def __start_ike(self):
        server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        server.bind((self.__interface, self.__ike_port))
        server.listen(8)
        ...
        while True:
            ...
            loop.create_task(self.__handle_ike(client))

    async def __handle_ike(self, client):
        loop = asyncio.get_event_loop()
        request = None
        while request != 'quit':
            print("test")
    ...
    def send_ike(self, data, to):
        print("test1")
        import sys
        sys.stdout.flush()
        self.test_server.sock_sendall(to, data.encode('utf8'))
